      ...
      >
        ...
        <div className="bg-white rounded-2xl shadow-2xl p-8">
          ...
          {/* OAuth Google */}
            ...
            className="w-full flex items-center justify-center gap-3 border border-gray-200 rounded-lg py-3 text-sm font-medium text-gray-700 hover:bg-gray-50 transition-colors mb-4"
          >
          ...
          <div className="relative my-4">
            ...
            <div className="relative flex justify-center text-xs text-gray-400">
              <span className="bg-white px-3">{t('login.orEmail')}</span>
            </div>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            {mode === "register" && (
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">{t('login.form.fullName')}</label>
                <input
                  type="text"
                  value={name}
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">{t('login.form.email')}</label>
              <input
                type="email"
                value={email}
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">{t('login.form.password')}</label>
              <input
                type="password"
                value={password}
            ...
            >
              {submitting ? t('login.loading') : mode === "login" ? t('login.submit.login') : t('login.submit.register')}
            </button>
          </form>
        </div>

        <p className="text-center text-blue-200 text-sm mt-6">
          <Link href="/"><a className="hover:text-white transition-colors">← {t('login.backToSite')}</a></Link>
        </p>
      </motion.div>
    </div>
  );
